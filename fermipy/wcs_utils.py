...
class WCSProj(object):
    ...
    def distance_to_edge(self, skydir):
        ...
        deltax = np.abs(deltax) - 0.5 * self._width[0].to(u.degree).value
        deltay = np.abs(deltay) - 0.5 * self._width[1].to(u.degree).value
...
def distance_to_edge(geom, skydir):
    ...
    deltax = np.abs(deltax) - 0.5 * geom.width[0].to(u.degree).value
    deltay = np.abs(deltay) - 0.5 * geom.width[1].to(u.degree).value
